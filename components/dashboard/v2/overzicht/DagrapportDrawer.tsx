...
/**
 * v2 Dagrapport-drawer: slide-in van rechts met de dag-statistieken (vandaag vs
 * gisteren). Port van de oude DagrapportDrawer naar de rebrand-stijl; hergebruikt
 * exact dezelfde DagrapportData (getDagrapport).
 *
 * Mount-pattern: de overzichtspagina rendert deze alléén bij `?dagrapport=1`.
 * Sluiten haalt de param weg via router.replace, zodat de URL de single source
 * of truth blijft (de back-knop sluit de drawer ook).
 */
export function DagrapportDrawer({ data }: { data: DagrapportData }) {
  ...
  // Portal naar de body (na mount) zodat de fixed-overlay niet door een
  // overflow-scroller wordt afgekapt. Tot mount: niets renderen (geen SSR-mismatch).
  ...
  // Body-scroll lock terwijl de drawer open is.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // close is referentieel niet stabiel; effect alleen bij mount/unmount.
  ...
  return createPortal(
    <div className="rbRoot">
      <div className={styles.overlay} onClick={close} aria-hidden />
      <aside className={styles.panel} role="dialog" aria-modal="true" aria-label="Dagrapport">
        <div className={styles.header}>
          <div>
            <div className={styles.headerTitle}>Dagrapport</div>
            <div className={styles.headerSub}>{datumFmt}</div>
          </div>
          <button type="button" className={styles.closeBtn} onClick={close} aria-label="Sluiten">
            <X size={16} strokeWidth={2.5} />
          </button>
        </div>

        <div className={styles.body}>
          {/* Surface-bot activiteit, bovenaan zodat het bot-narratief eerst komt. */}
            ...
            <div className={styles.surfaceCard}>
              <div className={styles.surfaceStat}>
                <span className={styles.surfaceLabel}>
                  <Send size={11} className={styles.surfaceIcon} />
                  Verstuurd
                </span>
              ...
              <div className={styles.surfaceStat}>
                <span className={styles.surfaceLabel}>
                  <MessageSquare size={11} className={styles.surfaceIcon} />
                  Ontvangen
                </span>
              ...
              <div className={styles.surfaceStat}>
                <span className={styles.surfaceLabel}>
                  <Clock size={11} className={styles.surfaceIcon} />
                  Reactietijd
                </span>
          ...
          <section className={styles.section}>
            ...
            <div className={styles.kpiGrid}>
              <KpiCard label="Nieuwe leads" value={data.vandaag.leads} prev={data.gisteren.leads} spark={data.sparklines.leads} />
              <KpiCard label="Offertes verstuurd" value={data.vandaag.offertesVerstuurd} prev={data.gisteren.offertesVerstuurd} spark={data.sparklines.offertes} />
              <KpiCard label="Akkoorden" value={data.vandaag.akkoorden} prev={data.gisteren.akkoorden} spark={data.sparklines.akkoorden} />
              <KpiCard label="Omzet akkoord" value={data.vandaag.omzet} prev={data.gisteren.omzet} spark={data.sparklines.omzet} formatEuro />
            </div>
          </section>
          ...
          <section className={styles.section}>
            ...
            {data.bronnen.length === 0 ? (
              <div className={styles.empty}>Nog geen leads vandaag, kanalen rusten even.</div>
            ) : (
...
/** Eén KPI-tegel met delta-pill + 7-daags sparkline. */
function KpiCard({
  label,
  value,
  prev,
  ...
  const display = formatEuro ? `€${Math.round(value).toLocaleString("nl-NL")}` : String(value);
  return (
...
/** Mini-sparkline (7 datapunten) als inline SVG. */
function Sparkline({ points }: { points: number[] }) {
  ...
  return (
    <svg className={styles.sparkWrap} viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" aria-hidden>
...
/** Uur-strip: 24 bars (0-23u), hoogte proportioneel aan event-count. */
        ...
        {data.map((count, hour) => {
          ...
          return (
            <div
              key={hour}
              className={cls}
              style={{ "--uur-h": ratio } as React.CSSProperties}
...
function Delta({ value, prev, formatEuro }: { value: number; prev: number; formatEuro: boolean }) {
  if (value === 0 && prev === 0) {
    return (
  ...
  const display = formatEuro ? `€${Math.round(abs).toLocaleString("nl-NL")}` : String(abs);
  return (
